...
public class TfliteHandler
{
    ...
    /**constructor*/
    public TfliteHandler(Context context, List<Bitmap> frames, List<String> names){
        mContext = context;
        mFrames = frames;
        mNames = names;
        initTfliteModel(context);
    }

    /** Memory-map the model file in Assets. */
    public static MappedByteBuffer loadModelFile(Context context) throws IOException {
        AssetFileDescriptor fileDescriptor = context.getAssets().openFd(InferenceHelper.LOCAL_MODEL_ASSET);
    ...
    private void initTfliteModel(Context activity){
        try {
    ...
    public InferenceHelper.Encoding convertToEncoding(Bitmap bmap, String fileName){
        if(bmap == null){
            Log.d("encoding", "ERROR : Trying to run inference on null bitmap!");
        }
        ...
        return new InferenceHelper.Encoding(mFaceEncodingOutput[0]);
    }
...
//    public void runTfliteInferenceOnAllCrops(Bitmap[] frames){
//        String cropsDirPath = mContext.getAssets().pa;
...
//
//        new AsycTfliteInferenceAll().execute(frames);
//    }
    ...
    public void processFrames(List<Bitmap> frames, List<String> fileNames){
        IntStream.range(0, Math.min(frames.size(), fileNames.size())).forEach(
                i -> {
                    runTfliteInference(frames.get(i),fileNames.get(i));
                    Log.d("progres_clustering",""+i);
                }
        );
    }
}