...
import  agh.mobile.blurfacesmcc.ui.uploadvideo.InferenceHelper;
    ...
    private class AsyncKmeans extends AsyncTask<Void, Integer, Void>{
        ...
        protected Void doInBackground(Void... voids) {
            ...
            for(int km = 0; km < mClusterIter; km ++) {
                List<KMeans.Mean> means = kmeans.predict(km, encodings);
...
//            MainActivity.clusteringProgressBar.setProgress(values[0]);
    ...
    private void getFloatEncodings(HashMap<String, InferenceHelper.Encoding> Encodings){
        ...
        while (it.hasNext()) {
            ...
            InferenceHelper.Encoding encoding = (InferenceHelper.Encoding) pair.getValue();
            System.arraycopy(encoding.enc, 0, encodings[e++], 0, DIM_ENCODING);
        }
    }

    public void KMeansClustering(HashMap<String, InferenceHelper.Encoding> Encodings){
...
//        new AsyncKmeans().execute();
    ...
    void doKameans(){
        ...
        for(int km = 0; km < mClusterIter; km ++) {
            List<KMeans.Mean> means = kmeans.predict(7, encodings);
            double score = KMeans.score(means);
            if (score > bestScore) {
                bestKMeans = means;
                bestScore = score;
...
//        MainActivity.clusterResultsText.setText(clusterOutputString);
    ...
    int getKMeansClusterIdx(InferenceHelper.Encoding encoding){
        KMeans.Mean nearestMean = KMeans.nearestMean(encoding.enc, bestKMeans);
        return bestKMeans.indexOf(nearestMean);
    }

    public void DBScanClustering(HashMap<String, InferenceHelper.Encoding> Encodings){
        ...
        while (it.hasNext()) {
            ...
            DoublePoint dbPoint;
            double[] p = new double[InferenceHelper.DIM_ENCODING];
            for(int i = 0; i < InferenceHelper.DIM_ENCODING; i++)
                p[i] = encoding.enc[i];
...
//        MainActivity.clusterResultsText.setText(clusterOutputString);
    ...
    int getDBScanClusterIdx(InferenceHelper.Encoding encoding){
